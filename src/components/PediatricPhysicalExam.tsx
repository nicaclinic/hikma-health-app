...
export const PediatricPhysicalExamDisplay = (metadataObj, language) => {
  return (
    <View>
      ...
      <Text>{LocalizedStrings[language].silvermanAnderson}: {metadataObj.silvermanAnderson}</Text>
      ...
      <Text>{LocalizedStrings[language].sizeLength}: {metadataObj.sizeLength}</Text>
      <Text>{LocalizedStrings[language].headCircunference}: {metadataObj.headCircunference}</Text>
      <Text>{LocalizedStrings[language].chestCircumference}: {metadataObj.chestCircumference}</Text>
      <Text>{LocalizedStrings[language].abdominalPerimeter}: {metadataObj.abdominalPerimeter}</Text>
      <Text>{LocalizedStrings[language].bodySurfaceArea}: {metadataObj.bodySurfaceArea}</Text>
...
const PediatricPhysicalExam = (props) => {
  const [bloodPressure, setBloodPressure] = useState(null);
  const [heartRate, setHeartRate] = useState(null);
  const [breathingFrequency, setBreathingFrequency] = useState(null);
  const [silvermanAnderson, setSilvermanAnderson] = useState(null);
  const [oxygenSaturation, setOxygenSaturation] = useState(null);
  const [weight, setWeight] = useState(null);
  const [sizeLength, setSizeLength] = useState(null);
  const [headCircunference, setHeadCircunference] = useState(null);
  const [chestCircumference, setChestCircumference] = useState(null);
  const [abdominalPerimeter, setAbdominalPerimeter] = useState(null);
  ...
  const submit = async () => {
    database.addEvent({
      ...
      visit_id: visitId,
      event_type: EventTypes.PediatricPhysicalExam,
    ...
    <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
      <View style={styles.containerLeft}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center', alignSelf: 'stretch', }}>
          <Text style={[styles.text, { fontSize: 16, fontWeight: 'bold' }]}>{LocalizedStrings[language].pediatricPhysicalExam}</Text>
        </View>
        ...
        <View style={styles.inputRow}>
          ...
          <TextInput
            style={styles.inputs}
            placeholder={LocalizedStrings[language].silvermanAnderson}
            onChangeText={(text) => setSilvermanAnderson(text)}
            value={silvermanAnderson}
            keyboardType='numeric'
          />
        </View>
        <View style={styles.inputRow}>
        ...
        <View style={styles.inputRow}>
          ...
          <TextInput
            style={styles.inputs}
            placeholder={LocalizedStrings[language].sizeLength}
            onChangeText={(text) => setSizeLength(text)}
            value={sizeLength}
            keyboardType='numeric'
          />
        ...
        <View style={styles.inputRow}>
          <TextInput
            style={styles.inputs}
            placeholder={LocalizedStrings[language].headCircunference}
            onChangeText={(text) => setHeadCircunference(text)}
            value={headCircunference}
            keyboardType='numeric'
          />
          <Text style={{ color: '#FFFFFF' }}>cm</Text>
          <TextInput
            style={styles.inputs}
            placeholder={LocalizedStrings[language].chestCircumference}
            onChangeText={(text) => setChestCircumference(text)}
        ...
        <View style={styles.inputRow}>
          <TextInput
            style={styles.inputs}
            placeholder={LocalizedStrings[language].abdominalPerimeter}
            onChangeText={(text) => setAbdominalPerimeter(text)}
        ...
        <View style={[styles.responseRow, { padding: 0 }]}>
          <TextInput
            ...
            numberOfLines={10}
            style={[styles.inputs]}
            onChangeText={(text) => setAttendsNameSign(text)}
            value={attendsNameSign}
          />
        </View>
        <View style={[styles.responseRow, { paddingVertical: 0 }]}>
...
export default PediatricPhysicalExam;
