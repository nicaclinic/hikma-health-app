...
export class DatabaseInitialization {
  ...
  private createTables(transaction: SQLite.Transaction) {
    ...
    transaction.executeSql(
      "CREATE TABLE IF NOT EXISTS patients (id varchar(32) PRIMARY KEY, given_name varchar(32) REFERENCES string_ids(id) ON DELETE CASCADE, surname varchar(32) REFERENCES string_ids(id) ON DELETE CASCADE, date_of_birth varchar(10), country varchar(32) REFERENCES string_ids(id), hometown varchar(32) REFERENCES string_ids(id) ON DELETE CASCADE, phone text, sex varchar(1), image_timestamp text,  medical_record_num text, local_id text, address text, attention_datetime text, attending_resources text, origin text, age varchar(10), email text, educational_status text, religion text, marital_status text, occupation text, mother_name text, father_name text, delivery_place text, delivery_datetime text, gestational_age text, delivery_care text, delivery_via text, presentation text, birthing_events text, edited_at text, deleted integer DEFAULT 0);"
    );
